...
func NewSpecsUseCases() *SpecsUseCases {
	...
	// Инициализируем спецификации для вражеских танков
	// В оригинальной Battle City враги обычно имеют одинаковые характеристики на всех уровнях
	for i := 0; i < 4; i++ {
		if i < len(uc.enemiesSpecs) {
			uc.enemiesSpecs[i] = types.NewSpecsEntity(
				uint(i), // Уровень врага
				32.0,    // Базовая скорость врага
				false,   // Пули не усиленные
				120.0,   // Скорость пули
				1,       // Лимит пуль: 1
			)
