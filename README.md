...
- **TankUseCases** - бизнес-логика танков (игрок/враги):
  ...
  - `Stop(byCollision)` - остановка танка (с коллизией или без)
...
- **TankBrakingService** - сервис торможения танка (вынесен из Use Cases):
  - `HandleBrakingState(dt)` - обработка движения в состоянии торможения
  - Танк доезжает до координаты кратной 4 при отпускании клавиши
  - Обработка смены направления во время торможения
  - Обработка возврата на 0.5 назад, если координата превышает кратное 4
